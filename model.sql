...
CREATE TABLE MEMB (
  ...
  CHECK CHAR(1)     NOT NULL COMMENT '인증여부', -- 인증여부
...
CREATE TABLE RV_CMT (
  ...
  MNO    INTEGER  NOT NULL COMMENT '회원일련번호', -- 회원일련번호
  MSNO   INTEGER  NOT NULL COMMENT '업체일련번호', -- 업체일련번호
  GROUP  INTEGER  NULL     COMMENT '그룹', -- 그룹
...
-- 찜
ALTER TABLE LIKE
  ADD CONSTRAINT FK_MEMB_TO_LIKE -- 회원 -> 찜
    FOREIGN KEY (
      MNO -- 회원일련번호
    )
    REFERENCES MEMB ( -- 회원
...
-- 찜
ALTER TABLE LIKE
  ADD CONSTRAINT FK_MEMB_STORE_TO_LIKE -- 업체 -> 찜
    FOREIGN KEY (
      MSNO -- 업체일련번호
    )
    REFERENCES MEMB_STORE ( -- 업체
